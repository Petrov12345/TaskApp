...
function ManageTeamPage() {
  ...
  const handleRenameTeam = () => {
    ...
    axios.post('http://localhost:5000/manage-team', {
      ...
      .then(() => {
        ...
        setOwnedTeams(ownedTeams.map(team => 
  ...
  const handleAddMember = (friendId) => {
    axios.post('http://localhost:5000/invite-to-team', {
      teamId: selectedTeam._id,
      inviteeId: friendId
    }, {
      headers: { Authorization: `Bearer ${token}` }
      ...
      .then(() => {
        ...
        setPendingInvites([...pendingInvites, { _id: friendId, username: invitedFriend.username }]);
      })
      .catch(error => {
        if (error.response?.status === 400 && error.response.data === "User already invited to this team") {
  ...
  const handleLeaveTeam = () => {
    if (window.confirm("Are you sure you want to leave this team?")) {
      axios.post('http://localhost:5000/leave-team', {
        teamId: selectedTeam._id
      }, {
        headers: { Authorization: `Bearer ${token}` }
      })
  ...
  const handleDeleteTeam = () => {
    if (window.confirm("Are you sure you want to delete this team?")) {
      axios.delete(`http://localhost:5000/delete-team/${selectedTeam._id}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
        .then(() => {
